...
bool_save_byrd_catchment_crop = False
bool_save_totten_crop = True
...
column_list = ["lon", "lat", "x", "y", "s", "t", "b", "b_inferred", "source", "trajectory_id", "trace_id"]
...
for v, (csv_list, folder_path) in enumerate(zip(list_of_all_versions, paths_to_data_folders_all_versions), start = 1):
    ...
    for i in csv_list:
        ...
        df.columns = ["lon", "lat", "s", "t", "b", "trajectory_id", "trace_id"]
...
if bool_save_totten_crop:
    # Hard-coded corners of 100 x 100 km Totten (crop). 
    x_min = 2_200_000
    x_max = 2_300_000
    y_min = - 1_050_000
    y_max = - 950_000

    # Subset the DataFrame to the Totten area
    bedmap123_data_totten = bedmap123_data[
        (bedmap123_data["x"] >= x_min) & (bedmap123_data["x"] <= x_max) &
        (bedmap123_data["y"] >= y_min) & (bedmap123_data["y"] <= y_max)
    ]

    output_path_totten = os.path.join(path_to_bedmap_data_folder, "bedmap123_totten_with_id.csv")
    bedmap123_data_totten.to_csv(output_path_totten, index = False)
    print(f"Totten area subset saved to {output_path_totten}.")